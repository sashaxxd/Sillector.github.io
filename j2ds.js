var j2Ds = (function() {
 ...
 j2Ds.input.onNode = function(_id) {
  //if (!_id.layer.visible) return false;j2Ds.scene.canvas.offsetLeft
  var coef2 = coeff/(last_scrin.my_width/my_width);
  var y_dop = 0;
  if(is_fullscrin && !is_fullscrin_just){
          y_dop = (-$("html").height() + $("body").height())/2*coef2 -10;
          console.log(y_dop);
      } 
  return (this.pos.x > _id.pos.x*coef2 && this.pos.x < _id.pos.x*coef2+_id.size.x*coef2) &&
         (this.pos.y > (_id.pos.y*coef2+y_dop) && this.pos.y < (_id.pos.y*coef2+y_dop)+_id.size.y*coef2);
 };
 ...
 j2Ds.input.onMove = function(e) {
  //console.log(this.screenPos.x);
  this.screenPos.x = -j2Ds.scene.canvas.offsetLeft+e.pageX;
  this.screenPos.y = -j2Ds.scene.canvas.offsetTop+e.pageY;
 };
 ...
 j2Ds.input.setCursorImage = function (_curImg) {
 	j2Ds.dom.tag('html')[0].style.cursor = 'url("'+_curImg+'"), auto';
 };
...
j2Ds.scene.layerObj = new Array();
 j2Ds.scene.reload_layer = function (_from, _to) {  
    for (var i =  0; i < j2Ds.scene.layerObj.length; i++) { 
      //j2Ds.scene.layerObj[i].setPosition_mult(j2Ds.scene.layerObj[i].getPosition1(),_to/_from);
      //j2Ds.scene.layerObj[i].setSize(v2df((parseInt(j2Ds.scene.layerObj[i].getSize().x/_from*_to*100))/100, (parseInt(j2Ds.scene.layerObj[i].getSize().y/_from*_to*100))/100));

      //alert(j2Ds.scene.layerObj[i].getPosition1().x)
  };}
 ...
 j2Ds.scene.initCanvas = function(_id, _canDeactivate) {
  ...
  j2Ds.dom.id(_id).oncontextmenu = function() { return false; };
  ...
  //alert(j2Ds.dom.id(_id).offsetLeft);
 ...
 j2Ds.scene.BaseNode.prototype.setPosition = function(_pos) {
  ...
  j2Ds.scene.BaseNode.prototype.setPosition1 = function(_pos) {
  if (_pos) {
   this.pos = j2Ds.math.v2f(_pos.x, _pos.y);
  } else {
   return this.pos;
  }
 };
  j2Ds.scene.BaseNode.prototype.setPosition_mult = function(_pos,_mult) {
  if (_pos) {
    this.setPosition1(j2Ds.math.v2f(parseInt(_pos.x*_mult*100)/100 , parseInt(_pos.y*_mult*100)/100));
  } else {
   return this.pos;
  }
 };
 ...
 j2Ds.scene.BaseNode.prototype.getPosition1 = function() {
  return j2Ds.math.v2f(this.pos.x, this.pos.y);
 };
